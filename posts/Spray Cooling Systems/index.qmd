...
# Define target variable Y and drop other columns from data
Y = pd.DataFrame(data['Nu'], columns=['Nu'])
data.drop(['P', 'um', 'Pr', 'rho', 'k', 'Nu', 'At'], inplace=True, axis=1)
...
# Print the random grid of hyperparameters
pprint(param_dist)

# Perform RandomizedSearchCV with 3-fold cross-validation and 100 iterations
random_search = RandomizedSearchCV(estimator=lgb_reg, param_distributions=param_dist, n_iter=100,
                                   cv=3, verbose=2, random_state=42, n_jobs=-1)
