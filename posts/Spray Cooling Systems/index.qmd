...
``` python
lnk = 'https://raw.githubusercontent.com/mlori77/ANN/main/heatml.csv'
...
X1 = df_model.drop(columns=['Nu','At','mio','k', 'um']).values
...
print("The tunned parameters are:", grid_result.best_params_)
...
X1 = df_model.drop(columns=['Nu','At','mio','k', 'um', 'Pr', 'P']).values
y1 = df_model.Nu.values
...
# Define model function without Dropout and with extended layers
def DNN_model(layers, lr_rate):
    DNN = Sequential()
    ...
    # Additional hidden layers (extended layers)
    for nodes in layers[1:]:
        DNN.add(Dense(nodes, kernel_initializer='uniform', activation="relu"))
        DNN.add(BatchNormalization())  # Batch Normalization
...
# GridSearchCV for hyperparameter tuning (or consider RandomizedSearchCV for better performance)
model = KerasRegressor(model=DNN_model, layers=layers, lr_rate=lr_rate, verbose=0)
...
mlp_result = pd.concat([pd.DataFrame(y_original), pd.DataFrame(y_test_original)], axis=1)
mlp_result.to_csv('ann_results.csv', index=False)
