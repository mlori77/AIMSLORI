...
rfr_result = pd.concat([pd.DataFrame(predict), pd.DataFrame(yt)], axis = 1)

rfr_result.to_csv('rfrnuht.csv')
```
...
svm_result.to_csv('svmnuhtgas_svm.csv')
```
...
dt_result = pd.concat([pd.DataFrame(predict), pd.DataFrame(yt)], axis=1)
dt_result.to_csv('dtnuht_decision_tree.csv')
```
